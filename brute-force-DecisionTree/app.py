...
from dt_detector import DTDetector
...
# Inisialisasi Decision Tree Detector dan Database
dt_detector = DTDetector(max_attempts=5, time_window=60)
db = Database()
...
def login():
    """Halaman login - Menggunakan Deteksi Decision Tree"""
    if request.method == 'POST':
        username = request.form.get('username', '').strip()
        password = request.form.get('password', '')
        ip_address = request.remote_addr
        
        # Verifikasi credentials DULU ke SQLite
        is_valid = db.verify_credentials(username, password)
        
        # Record login attempt dengan model Decision Tree
        result = dt_detector.record_login_attempt(username, ip_address, is_valid)
        
        # Ambil nilai string state hasil prediksi ('normal', 'suspicious', 'blocked')
        current_state = result['state']
        
        # Log ke database log lokal
        db.log_login_attempt(username, is_valid, ip_address,
                             current_state, result['message'])
        
        # Handle berdasarkan hasil prediksi model
        if is_valid and current_state != 'blocked':
            # Login berhasil dan tidak diblokir oleh Decision Tree
...
def dashboard():
    ...
    # Ambil statistik dari modul Decision Tree
    stats = dt_detector.get_statistics(username=username, ip_address=ip_address)
    detailed_stats = dt_detector.get_detailed_stats()
...
def api_stats(username):
    """API endpoint untuk mendapatkan statistik berbasis Decision Tree"""
    ip_address = request.remote_addr
    stats = dt_detector.get_statistics(username=username, ip_address=ip_address)
...
def api_system_stats():
    ...
    stats = dt_detector.get_detailed_stats()
    return jsonify(stats)
...
if __name__ == '__main__':
    # Anda bisa ubah port ke 5001 agar tidak tabrakan dengan aplikasi FSA utama saat demo
